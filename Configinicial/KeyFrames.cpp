...
float bodyTilt = 0.0f; // Inclinación del cuerpo
...
void saveFrame(void)
{
	...
	//Para el frame de la cabeza 
	KeyFrame[FrameIndex].head = head;

	//Para el frame de las patas
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Skybox - Miriam Lopez", nullptr, nullptr);
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotDog = 0.0f;
		KeyFrame[i].rotDogInc = 0.0f;

		KeyFrame[i].head = 0.0f;
		KeyFrame[i].headInc = 0.0f;

		KeyFrame[i].FLeftLeg = 0.0f;
		KeyFrame[i].FLeftLegInc = 0.0f;

		KeyFrame[i].FRightLeg = 0.0f;
		KeyFrame[i].FRightLegInc = 0.0f;

		KeyFrame[i].BLeftLeg = 0.0f;
		KeyFrame[i].BLeftLegInc = 0.0f;

		KeyFrame[i].BRightLeg = 0.0f;
		KeyFrame[i].BRightLegInc = 0.0f;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Guardamos la matriz base para cabeza, cola y patas
		modelTemp = model;
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
